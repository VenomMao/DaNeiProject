...
<link href="../css/goods_details.css" rel="stylesheet" />
</head>
<body>
	<!-- 顶部区域 -->
	...
	<div id="shop_detail">
		...
		<div class="right_detail lf">
			...
			<div class="price">
				<div id="pro_price">
					<b>学员售价：</b><span>￥${goods.price }.00</span>
				</div>
				<div class="promise">
			...
			<!-- 颜色-->
			<p style="display: none;" class="style" id="choose_color">
				<s class="color">颜色：</s> <input type="button" class="i1" value="黑色"
					title="黑色" /> <input type="button" class="i2" value="灰色" title="灰色" />
				<input type="button" class="i3" value="金色" title="金色" /> <input
			...
			<div class="avenge">
				<ul>
					<c:forEach items="${goodsList }" var="goodsItem">
						<li><a href="?id=${goodsItem.id }"
							<c:if test="${goodsItem.id == goods.id }">
	            			class="current"
	            		</c:if>>${goodsItem.title }</a></li>
					</c:forEach>
				</ul>
			</div>
			...
			<form id="buyForm">
				<s>数量：</s> <input type="button" value="-" class="numberMinus" /> <input
					type="button" value="+" class="numberAdd" /> <input type="text"
					value="1" class="number" id="buy-num" name="goodsCount"> <input
					type="hidden" name="goodsId" value="${goods.id }" /> <input
					type="hidden" name="goodsImage" value="${goods.image }" /> <input
					type="hidden" name="goodsTitle" value="${goods.title }" /> <input
					type="hidden" name="goodsItemType" value="${goods.itemType }" /> <input
					type="hidden" name="goodsPrice" value="${goods.price }" />
			</form>
			</p>
			<!-- 购买部分-->
			<div class="shops">
				<a href="cart.html" class="buy lf" id="buy_now">立即购买</a> <a
					href="###" class="shop lf" id="add_cart"> <img
					src="../images/product_detail/product_detail_img7.png" alt="" />
					加入购物车
				</a> <a href="#" class="collection lf" id="collect"><span>收藏</span></a><b><img
	...
	<script>
		$(function() {
			...
			$("#add_cart").click(function(e) {
				// 发出请求
				// 将当前页面显示的商品添加到购物车
				var url = "../cart/add.do";//线条出当前路径，从goods/xxx调到cart/xxx
