...
<div id="popup_content">
	...
	<form id="address-form" method="post" action="">
		...
		<!--<input type="hidden" name="id" id = "id"/> 
		    给name属性是希望表单序列化时候用，给id是希望JQ里面去取值  -->
		<div class="buttons-block">
		<input type="hidden" name="id" id = "id"/>
      	<a href="#"
      		onclick="postForm()"
...
var htmlTemplate = '<div class="content %CONTENT_ACTIVE%">'
			...
			+ '</span>'
			+ '<span class="set_default">' 
			+ '<a href="###" style="display: %SET_DEFAULT%;" onclick="setDefault(%ID%)">设为默认</a>'
			+ '</span>'
			+ '</div>';
...
function showPopup(id) {
	//清空表单中各控件已有的值
	//jq取到的对象调用原生的js函数，必须加一个下标、调用自己的jq不用。
	...
	//发出ajax请求获取需要编辑的数据，并显示到各个控件中
	//val一般输入框 表单中有value控件用  html一般标签希望里面有什么用。
	
	//id是showPop里面的参数id,id=0是新增
	if(id!=0){
		//编辑修改操作
		var url = "get.do";
		var data = "id="+id;
		$.ajax({
			...
			"success":function(obj){
				...
				//// 加载省的列表
				getProvinces(address.recvProvince,
						address.recvCity,address.recvArea);
